...
export const useConversationStore = defineStore({
    ...
    actions: {
        ...
        async createConversation(form) {
            const conversation = await ConversationLogic.createConversation({ ...form, completed: true, maxParticipants: 2, type: 'conversation' });
            this.conversations.push(conversation);
            this.selectedConversationId = conversation.id;
            return conversation;
        },
        async createRoom(form) {
            await ConversationLogic.createRoom({ ...form, type: 'room' }).then((data) => {
        ...
        async getConversations() {
            await ConversationLogic.getConversations()
                .then((data) => {
        ...
        async getConversationOfUser(userId) {
            const conversation = await ConversationLogic.getConversationOfUser(userId);
